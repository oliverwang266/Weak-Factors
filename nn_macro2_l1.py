# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 16:55:17 2024
...
def create_model(hidden_units=[4], lambda_value=10,epochs=100, init_var=0.01):
    np.random.seed(an)
    random.seed(an)
...
def fit_models(X, y, index_train, index_pred):
    ...
    # nn_grid = {
    #     'lambda_value': np.arange(0.01, 0.1, 0.02)
    # }
    nn_grid = {
        'epochs':[25,50,100,200],
        'lambda_value': 10**np.arange(-4, 2+0.1, .5)
    } 
    ...
    f=open("weak/macro2_l1_result.txt","a")
...
'''
file_path = 'macro2_l1_result.txt'  
