...
def create_model(hidden_units=[16], lambda_value=10, epochs=100, init_var=0.01):
    ...
    model.add(Dense(hidden_units[0], input_dim=X.shape[1]-1, kernel_regularizer=l2(lambda_value), 
...
def fit_models(i):
    ...
    #for lbd in np.arange(0.001, 0.01, 0.002):
    model = KerasRegressor(build_fn=create_model, batch_size=16, verbose=0)
    # nn_grid = {
    #     'lambda_value': [0.00095, 0.001, 0.00105]
    # }
    nn_grid = {
        'hidden_units': [[8]],
        'lambda_value': 10**np.arange(-1,-0.5+0.01,.05), 
        'epochs':[75,150,300,600]  
    }
    ...
    f=open("weak/macro1b_l2_result.txt","a")
...
file_path = 'macro1b_l2_result.txt'  # Replace with the path to your file
