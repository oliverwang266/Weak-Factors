...
def fit_models(i):
    ...
    f=open("weak/macro1b_xgb_result.txt","a")
    f.write(f"{an} {((y_pred - y_predb-y_predhat) ** 2).mean()} { ((y_pred - y_predb) ** 2).mean()} {best_params['max_depth']} {best_params['learning_rate']} {best_params['n_estimators']}"+'\n')

    # Return the MSE
    return [
        ((y_pred - y_predhat - y_predb) ** 2).mean(),
        ((y_pred - y_predb) ** 2).mean()
    ]
...
'''
file_path = 'macro1b_xgb_result.txt'  
