# -*- coding: utf-8 -*-
"""
Created on Sun Jan 28 10:39:45 2024
...
def create_model(lbd, epochs):
    ...
    model.add(Dense(32, 
                    activation='linear', 
                    input_shape=(920,), 
                    kernel_regularizer=regularizers.l1(lbd),
...
NN1m_df=pd.DataFrame(columns=feature_names)
resultpath = f"weak/finance2_l1_resultnew2.txt"
def myfunc(year):
  ...
  #  yscale=np.array([0.33,0.33,1,50,10,50,5,50])
    ...
    del xtrain, ytrain
    r2_NN1m=1-np.sum(np.power(nn_pred-yoos,2))/np.sum(np.power(yoos,2))
    vip_scores_NN1m={}
    for i, feature in enumerate(feature_names):
      ...
      # 对每个模型进行预测并计算MSE
      yhat_NN1m_mod = final_model.predict(xoos_mod) +ymean

      #计算每个模型新的R2
      r2_NN1m_mod=1-np.sum(np.power(yhat_NN1m_mod-yoos,2))/np.sum(np.power(yoos,2))
      #计算每个模型的R2变化
      vip_NN1m=r2_NN1m_mod-r2_NN1m

      # 将每个模型的R2变化添加到字典中
      vip_scores_NN1m[feature] = vip_NN1m
      del xoos_mod,yhat_NN1m_mod 
    NN1m_df.loc[year] = vip_scores_NN1m
    filename='weak/Finance2_VIP/NN1mnew2_df.csv'
    if os.path.exists(filename):
      existing_df=pd.read_csv(filename, index_col=0)
      existing_df.loc[year]=NN1m_df.loc[year]
      existing_df.to_csv(filename)
    else:
       NN1m_df.loc[[year]].to_csv(filename)
    ...
    combined_data=np.column_stack((oosdate,nn_pred,yoos))
    filename1 = 'weak/Finance2_VIP/Finance2_l1_prednew2.txt'
    # 使用追加模式打开文件
...
'''
file_path = 'finance2_l1_resultnew.txt'  
